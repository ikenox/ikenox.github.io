...
- Repositoryは単に`Hoge`オブジェクトを現在の状態のままDBに突っ込んだり取り出したりするだけである。そのため、`Hoge`の内部状態がいつどのように変化するかなどはRepositoryは全く知らなくてよい。
...
- ロジックが散らばらない。「`Hoge`テーブルの各値は、どういう条件・状態のときにどういう値に書き換わり、どこでどう使われるのか」は、Hogeオブジェクトのロジックの内部を見れば大体把握できる
